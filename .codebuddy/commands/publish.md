...
> **重要**：禁止仅用 `list_dir` 对比当前目录列表来判断——它只看顶层，无法发现内部文件移动/重命名（如 `components/` → `common/components/`）。必须用 git diff 捕获所有增删改移。

操作步骤：

1. `git diff v{当前版本号}..HEAD --name-status`
2. 根据 diff 结果判断 `src/` 下的目录/文件变更是否影响 README 的目录结构树：
   - **目录结构无变化** → 跳过
   - **仅有目录结构小幅调整**（如新增/删除几个文件、路径微调） → 直接更新
   - **有重大变更**（目录拆分、重组、模块层级变动） → 列出变更点并展示菜单：
