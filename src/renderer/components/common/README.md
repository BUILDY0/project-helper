...
| `base-select`         | 下拉选择                | `modelValue`、`options`([{label,value,disabled?}] 或 [{group,options:[...]}])、`placeholder`、`disabled`、`clearable`、`multiple`、`filterable`、`reserveKeyword`、`allowCreate`(配合 filterable 时按搜索词新增选项)、`size`(sm/md)；emit：`change(value, option)` / `create(value)`；slot：`#option={option,selected}`；expose：`focus()` |
| `base-tag`            | 标签 chip               | `label`、`hash`(前置 #)、`closable`(尾部 × → emit `close`)、`clickable`、`size`(sm/md)                                                                                                                                                                                                                                                     |
